...
extract_countries <- function(html) {
    ...
    count_h2 <- html %>%
        html_elements("h2") %>%
        map(as.character) %>%
        keep(.p = str_detect, pattern = "edit") %>%
        length()
...
extract_managers <- function(file) {
    ...
    indexes <- text %>%
        str_detect("(Head coach(es)?|Coach(es)?|Managers?|Head cocah(es)?):")
    p <- p[indexes]

    # Fix error in Wikipedia
    if (file == "data-raw/Wikipedia-squad-pages/1934_world_squads.html") {
...
wikipedia_squads <- wikipedia_squads %>%
    mutate(
        player_name = case_when(
            ...
            player_name == "Egidio Arévalo Ríos" ~ "Egidio Arévalo",
            TRUE ~ player_name
        )
    )
...
wikipedia_squads <- wikipedia_squads %>%
    mutate(
        player_wikipedia_link = case_when(
            ...
            player_wikipedia_link == "/wiki/Giorgian_De_Arrascaeta" ~ "/wiki/Giorgian_de_Arrascaeta",
            player_wikipedia_link == "/wiki/Ricardo_Rodr%C3%ADguez_(footballer)" ~ "/wiki/Ricardo_Rodriguez_(footballer)",
            player_wikipedia_link == "/wiki/Munir_Mohand_Mohamedi" ~ "/wiki/Munir_Mohamedi",
            player_wikipedia_link == "/wiki/Maicon_Sisenando" ~ "/wiki/Maicon_(footballer,_born_1981)",
            player_wikipedia_link == "/wiki/Egidio_Ar%C3%A9valo_R%C3%ADos" ~ "/wiki/Egidio_Ar%C3%A9valo",
            TRUE ~ player_wikipedia_link
        )
    )
...
player_ids <- wikipedia_squads %>%
    ...
    ungroup() %>%
    mutate(
        player_id = base::sample(1:100000, size = n(), replace = FALSE),
        player_id = str_c(
            "P-",
...
# Early years don't have position
table(is.na(wikipedia_squads))
map_dbl(wikipedia_squads, ~sum(is.na(.)))
# wikipedia_squads %>% filter(is.na(position_name))
...
# Save data
write_rds(wikipedia_squads, file = "data-raw/Wikipedia-data/wikipedia_squads.rds")
write_rds(wikipedia_managers, file = "data-raw/Wikipedia-data/wikipedia_managers.rds")
