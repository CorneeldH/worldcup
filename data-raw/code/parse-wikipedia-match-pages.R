...
extract_team_data <- function(html_table) {
    ...
    # Some hardcoded corrections to make sure links is same length as table
    
    # Interim manager of Jamaica has no wikipedia page
    links_jamaica_2016_manager_with_no_wikipedia <- c(
        "/wiki/Andre_Blake",
        "/wiki/Je-Vaughn_Watson",
        "/wiki/Adrian_Mariappa",
        "/wiki/Wes_Morgan",
        "/wiki/Jermaine_Taylor_(footballer)",
        "/wiki/Garath_McCleary",
        "/wiki/Michael_Hector",
        "/wiki/Lee_Williamson",
        "/wiki/Jobi_McAnuff",
        "/wiki/Clayton_Donaldson",
        "/wiki/Giles_Barnes",
        "/wiki/Michael_Binns",
        "/wiki/Dever_Orgill"
    )
    ...
    ## Add a fake link
    if (identical(links, links_jamaica_2016_manager_with_no_wikipedia)) {
        links <- append(links, "/wiki/Miguel_Coley")
    }
    
    ## Iceland has two managers, combine the links
    if (nth(links, -2) == "/wiki/Heimir_Hallgr%C3%ADmsson" & 
        nth(links, -1) == "/wiki/Lars_Lagerb%C3%A4ck") {
        combined <- "/wiki/Heimir_Hallgr%C3%ADmsson /wiki/Lars_Lagerb%C3%A4ck"
        links <- links[-c(length(links)-1, length(links))]
        links <- append(links, combined)
    ...
    if(!is.null(home_team_goal_data)) {
        ...
        home_team_goals <- tibble(
            ...
            text = home_team_goal_data %>%
                ...
                str_remove_all("\\[[0-9]+\\]") %>%
                str_squish() %>%
                keep(.p = str_detect, pattern = "[0-9]"),
            player_wikipedia_link = home_team_goal_data %>%
                html_elements("a") %>%
                html_attr("href") %>%
                str_squish() %>%
                discard(.p = str_detect, pattern = "Penalty.kick|Own.goal|#cite_note")
        )
    } else {
...
extract_match_data <- function(file) {
    ...
    if (startsWith(page_name, "Copa América Centenario")) {
        page_name <- str_replace(page_name, "Centenario", "2016")
    }
    ...
    # Add code for a game that wasn't played due to the Anschlüss of Austria
    if (year == 1938) {
        home_teams <- home_teams[-3]
        away_teams <- away_teams[-3]
        dates <- dates[-3]
        matches <- matches %>% filter(match_id != "Sweden v Austria (1938-06-05)")
    }
...
# Save tables
write_rds(wikipedia_matches, file = "data-raw/Wikipedia-data/wikipedia_matches.rds")
write_rds(wikipedia_goals, file = "data-raw/Wikipedia-data/wikipedia_goals.rds")
write_rds(wikipedia_penalty_kicks, file = "data-raw/Wikipedia-data/wikipedia_penalty_kicks.rds")
write_rds(wikipedia_lineups, file = "data-raw/Wikipedia-data/wikipedia_lineups.rds")
write_rds(wikipedia_referees, file = "data-raw/Wikipedia-data/wikipedia_referees.rds")
